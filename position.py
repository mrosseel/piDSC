...
class Positioner:
    ...
    def euler_to_alt_az(self, psi, theta, phi):
    ...
    def getCurrentAltAz(self, bno):
        ...
        roll_x, pitch_y, yaw_z = self.quaternion_to_euler(qx, qy, qz, qw);
        #print("Euler: roll: %0.6f, pitch: %0.6f, yaw: %0.6f" % (roll_x, pitch_y, yaw_z))

        # convert euler to alz/az (radians)
        alt_rad, az_rad = self.euler_to_alt_az(roll_x, pitch_y, yaw_z)
    ...
    def getCurentRaDec(self, bno, earthLocation):
        ...
        alt, az = self.getCurrentAltAz(bno)
        ...
        currentPos = SkyCoord(alt = alt*u.deg, az = az*u.deg, obstime = t, frame = 'altaz', location = earthLocation)

        # global offset
        # if (offset is None):
        #     print("storing offset")
        #     offset = currentPos.skyoffset_frame()
        # else:
        #     print("applying offset")
        #     currentPos = currentPos.transform_to(offset)

        currentPos = currentPos.transform_to('icrs')

        # global offset
        # if (offset is not None):
        #     print("Calculating offset")
        #     offset_angle = currentPos.position_angle(offset).to(u.deg);
        #     offset_distance = currentPos.separation(offset).to(u.arcmin);
        #     print(offset_angle)
        #     print(offset_distance)
        #     #print("Offset: %0.3f° %0.3f arcmin" % (offset_angle, offset_distance))
    ...
    def getLX200_RADEC(self):
        # HH:MM:SS#
        ra_hms = self.current_radec.ra.hms
        ra_str = ("{:02d}:{:02d}:{:02d}#").format(int(ra_hms.h),int(ra_hms.m),int(ra_hms.s))

        # sDD*MM’SS#
        dec_dms = self.current_radec.dec.dms
        dec_str = ("{:+02d}*{:02d}’{:02d}#").format(int(dec_dms.d),int(dec_dms.m),int(dec_dms.s))
    ...
    def radecLoop(self):
        ...
        offset = None
        # getting initial value because the startup process is so slow
        self.getCurentRaDec(bno, loc).transform_to('icrs')
        ...
        while True:
            ...
            with self._lock:
                local_radec = self.current_radec
                local_radec = self.getCurentRaDec(bno, loc)
                self.current_radec = local_radec
            ...
            # print("thread RA: " + self.current_radec.ra.to_string(u.hour))
            # print("thread DEC: " + self.current_radec.dec.to_string(u.degree, alwayssign=True))
